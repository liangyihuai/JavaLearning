...
public final class HilbertCurve {
     ...
     *
     * @param bits depth of the Hilbert curve. If bits is one, this is the top-level
     *            Hilbert curve
     ...
     *
     * @param bits
     *             along the Hilbert curve in transposed form
    ...
    public BigInteger toIndex(long... transposedIndex) {
    ...
    public static void main(String[] args) {
        int order =1;
        HilbertCurve c = HilbertCurve.bits(order).dimensions(2);
        int pointNum = (int)Math.pow(2, (2*order));
        int sideLength = (int)Math.pow(2, order);

        System.out.println("Original linear data: size is = "+pointNum);
        System.out.print("Display: [1, ..., "+pointNum+"]: ");
        ...
        int [][] nOrderMatrix = new int[sideLength][sideLength];
        for(int i = 0; i < pointNum; i++){
            long[] ti = c.point(BigInteger.valueOf(i));
            int x = (int)ti[0];
            int y = (int)ti[1];
            nOrderMatrix[x][y] = i+1;
        }
